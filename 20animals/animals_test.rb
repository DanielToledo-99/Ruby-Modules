...
class AnimalsTest < Minitest::Test
  def test_can_create_animal
    # skip
    animal = Animal.new({ species: "some species", name: "some name" })
    assert_instance_of(Animal, animal)
  end

  def test_species_and_name_are_not_required
    # skip
    animal = Animal.new
    assert_instance_of(Animal, animal)
  end

  def test_species_and_name_are_set_unknown_when_not_given
    # skip
    animal = Animal.new
    ...
    assert_match '@species="Unknown"', output
    assert_match '@name="Unknown"', output
  end

  def test_can_read_species_and_name
    # skip
    animal = Animal.new({ species: "some species", name: "some name" })
    assert_equal animal.species, "some species"
    assert_equal animal.name, "some name"
  end

  def test_puts_some_animal_respond_with_custom_message
    # skip
    animal = Animal.new({ species: "monkey", name: "george" })
  ...
  def test_a_horse_species_is_set_by_default
    # skip
    horse = Horse.new("caballin")
    assert_equal "caballus", horse.species
  end
  ...
  def test_a_tiger_species_is_set_by_default
    # skip
    tiger = Tiger.new("tigrillo")
    assert_equal "tigris", tiger.species
  end
  ...
  def test_runner_module_run_method_access_class_species_and_return_correct_message
    # skip
